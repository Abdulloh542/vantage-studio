      ...
      <div className="max-w-[1440px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 pb-16 border-b border-white/12">
          ...
          <div className="col-span-1 md:col-span-2 space-y-4">
            ...
            <p className="font-sans text-base sm:text-lg text-white/70 leading-relaxed max-w-lg font-light">
          ...
          <div className="col-span-1 flex items-start md:justify-end">
            <Link
              to="/contact"
              className="solum-btn px-6 py-3.5 border border-white text-white text-xs sm:text-sm font-mono uppercase tracking-wider hover:bg-white hover:text-[#111111] transition-colors duration-180"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0 divide-y sm:divide-y-0 sm:divide-x divide-white/12 pt-16">
          {displayServices.map((srv, idx) => (
            <div
              key={srv.slug}
              className="p-6 md:p-8 flex flex-col justify-between min-h-[400px] group hover:bg-white/[0.03] transition-colors duration-300"
            >
              <div>
                <span className="font-mono text-xs sm:text-sm text-white/50 block mb-8 font-medium">
                  0{idx + 1} // DISCIPLINE
                </span>

                <h3 className="font-display text-2xl sm:text-3xl font-semibold tracking-[-0.04em] text-white uppercase leading-tight mb-4 group-hover:text-white">
                  {srv.title}
                </h3>

                <p className="font-sans text-sm sm:text-base text-white/70 leading-relaxed mb-6 font-light">
                  {srv.shortDescription}
                </p>
              </div>

              {/* Concise Deliverable Bullets */}
              <div className="pt-6 border-t border-white/12">
                <ul className="space-y-2.5">
                  {srv.deliverables.slice(0, 3).map((d) => (
                    <li key={d.name} className="flex items-center gap-2.5 text-xs sm:text-sm font-mono text-white/80">
